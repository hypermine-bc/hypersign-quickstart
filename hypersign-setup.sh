...
usage() {
    ...
    echo  
    echo "Syntax: hypersign-setup.sh -r <REDIRECT_URI> -a <CLIENT_ALIAS> -u <KC_USERNAME> -p <KC_PASSWORD>"
    ...
    echo "    -u    | --username              Provide keycloak username"
    echo "    -p    | --password              Provide keycloak password"
...
# docker cp hs-script/kc-webcontext.sh "$(docker-compose -f docker-compose.yml ps -q keycloak)":$CONFIGPATH/kc-webcontext.sh
docker cp hs-script/hs-plugin-install.sh "$(docker-compose -f docker-compose.yml ps -q keycloak)":$CONFIGPATH/hs-plugin-install.sh
docker cp hs-script/kc-configuration.sh  "$(docker-compose -f docker-compose.yml ps -q keycloak)":$CONFIGPATH/kc-configuration.sh

docker cp data-template/authenticator-flow-update.json "$(docker-compose -f docker-compose.yml ps -q keycloak)":$CONFIGPATH/authenticator-flow-update.json
docker cp data-template/client-create.json  "$(docker-compose -f docker-compose.yml ps -q keycloak)":$CONFIGPATH/client-create.json
docker cp data-template/client-update.json  "$(docker-compose -f docker-compose.yml ps -q keycloak)":$CONFIGPATH/client-update.json

echo -e "${BLUE_BG}Running keycloak web context script${NC}"
# docker-compose -f docker-compose.yml exec --user root keycloak sh $CONFIGPATH/kc-webcontext.sh
...
then
  ...
  docker-compose -f docker-compose.yml exec  keycloak sh $CONFIGPATH/hs-plugin-install.sh
...
docker-compose -f docker-compose.yml exec --user root keycloak sh $CONFIGPATH/kc-configuration.sh ${REDIRECT_URI} ${CLIENT_ALIAS} ${NO_PASSWORDLESS} ${KC_USERNAME} ${KC_PASSWORD}
